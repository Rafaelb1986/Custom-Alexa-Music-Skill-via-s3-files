...
DATASOURCE = {
    "audioPlayerTemplateData": {
        ...
        "properties": {
            ...
            "audioSources": [
                create_presigned_url("Media/freeyourmind.mp3")
            ],
            "backgroundImage": create_presigned_url("Media/night.jpg"),
            "coverImageSource": create_presigned_url("Media/freeyourmind.jpg"),
        ...
        if self.supports_apl(handler_input):
            handler_input.response_builder.add_directive(
                RenderDocumentDirective(
                    ...
                    document={
                        "type": "Link",
                        "src": f"doc://alexa/apl/documents/playmusic"
                    },
            ...
            PlayDirective(
                ...
                audio_item=AudioItem(
                    stream=Stream(
                        ...
                        url=create_presigned_url("Media/freeyourmind.mp3"),
...
class PauseIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        # Check if the device supports APL and clear the screen if necessary
        if get_supported_interfaces(handler_input).alexa_presentation_apl is not None:
            handler_input.response_builder.add_directive(
                RenderDocumentDirective(
                    token="clearScreenToken",
                    document={
                        "type": "APL",
                        "version": "1.0",
                        "mainTemplate": {
...
class ResumeIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        # Add directive to play audio from the last stopped position
        handler_input.response_builder.add_directive(
            PlayDirective(
                play_behavior=PlayBehavior.REPLACE_ALL,
                audio_item=AudioItem(
                    stream=Stream(
                        token="midnight_radio",
                        url=create_presigned_url("Media/freeyourmind.mp3"),
                        offset_in_milliseconds=0  # Adjust this value if you have the last stopped position
                    ),
                    metadata=AudioItemMetadata(
                        title="Midnight Radio",
                        subtitle="Enjoy the music"
        ...
        # Check if the device supports APL and update the screen if necessary
        if get_supported_interfaces(handler_input).alexa_presentation_apl is not None:
            handler_input.response_builder.add_directive(
                RenderDocumentDirective(
                    token="documentToken",
